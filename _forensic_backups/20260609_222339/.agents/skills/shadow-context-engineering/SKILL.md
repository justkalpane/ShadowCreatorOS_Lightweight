...
- scene sync matrix whenever the output claims production-ready Media Factory
  context, even if provider execution remains planning-only
- production order lock, asset inventory ledger, asset dependency graph,
  control panel execution plan, DaVinci timeline packet, and production proof
  gate whenever the task moves from script context into a real visual/media
  generation plan
...
- Do not call a production-ready media-context packet complete if the route
  stops at separate context blocks without `SCENE_SYNC_MATRIX`.
