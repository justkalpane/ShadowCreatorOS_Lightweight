...
## Repo Consumption Loop Blocker

- Boot stage runs once per task, then the route state owns recovery.
- After `ROUTE_LOCKED`, do not reread unchanged startup files unless the
  reread reason is one of: `file_hash_changed`, `audit_mode`, `validator_mode`,
  `explicit_user_requested_compare`, `route_manifest_changed`, or
  `compaction_recovery_validation`. Selected-director dependency tracing and
  semantic influence verification are also valid reasons when declared.
- After `DEPENDENCIES_CONSUMED`, set `output_phase_started=true` and generate
  the deliverable. Continuing repo reads for file-count compliance is drift.
- After compaction, resume from `runtime/state/route_state_contract.md`
  semantics: verify `route_manifest_hash`, keep `files_consumed`, and continue
  at `next_required_step`.
- `outputs/missions/**` is historical by default. Use it only in
  `continue_previous_mission`, `regression_mode`, or explicit user reference.
- `downloads/chat_transcript.txt` is drift-audit evidence only. Do not treat it
  as active runtime for a fresh script route.
- For `task_mode=script_only`, block Media Factory final-draft and
  local/cloud execution contracts until after `FINAL_SCRIPT` unless the user
  asks for visual planning, generator draft, or Media Factory handoff.
...
Synthetic route evidence is drift. Do not write or imply consumption ledgers for
files that were not actually read in the current task. If the route manifest,
mandatory contracts, selected directors, agents, subagents, skills, or subskills
cannot be read, stop with `BLOCKED_BEFORE_OUTPUT`; do not produce a shortened
script, shallow visual plan, or "internally evaluated" ledger.
...
For `SCRIPT_GENERATION`, `route_scope_status=PASS` without a
`ROUTE_SCOPE_FILE_AUDIT` is drift. The audit must prove selected route manifest
read, selected route slice read, mandatory route-slice paths consumed, semantic
influence present, output phase started, and final script generated. File
counts are telemetry only. Summarizing names such as "Krishna, Aruna..." is not
enough.
