...
Boot order is not a loop. Once the task is classified and the selected route is
locked, keep a route-state capsule with file hashes and read counts. Reuse that
capsule after compaction instead of restarting startup reads. Reread boot files
only when `reread_reason` proves the selected route, validator, director, hash
change, audit, or semantic influence check needs it.

`ROUTE_SCOPE_FILE_AUDIT` proves correct scope, but file counts alone never prove
semantic use. Every selected mandatory component must appear in a semantic
influence map tied to a real output block.

For `task_mode=script_only`, Media Factory contracts, old mission outputs, and
chat transcripts are blocked until after `FINAL_SCRIPT` unless the user
explicitly requests the downstream route or audit mode.

Do not synthesize missing route consumption from memory, role names, or registry
summaries. If the full selected route scope cannot be read, output
`BLOCKED_BEFORE_OUTPUT` with missing repo-relative paths instead of generating a
shortened artifact.
...
For `SCRIPT_GENERATION`, require a visible `ROUTE_SCOPE_FILE_AUDIT` before
`FINAL_SCRIPT`. PASS requires `canonical_route_id=SCRIPT_GENERATION`,
selected route manifest read, selected route slice read, mandatory route-slice
paths consumed, semantic influence present, output phase started, final script
generated, and missing scope set to `None`. File counts are telemetry only.
