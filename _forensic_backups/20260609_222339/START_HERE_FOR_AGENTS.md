...
Do not reread unchanged boot files after `ROUTE_LOCKED` without a visible
`reread_reason`. Rereads are allowed when a selected director, validator, hash
change, audit mode, or semantic influence check genuinely needs them. Do not
restart boot after compaction unless `boot_state_invalid=true`; resume from
route state, verify `route_manifest_hash`, and continue at `next_required_step`.

For a plain script request, default to `task_mode=script_only`. Do not load
Media Factory final-draft contracts, local/cloud execution contracts,
`outputs/missions/**`, or `downloads/chat_transcript.txt` before `FINAL_SCRIPT`
unless the user asks for visual planning, Media Factory handoff, regression
audit, or previous mission continuation.

When selected-route dependencies are consumed, start output phase instead of
continuing repo scans:
...
For supported production routes, route slices are mandatory. For
`SCRIPT_GENERATION`, PASS requires a visible `ROUTE_SCOPE_FILE_AUDIT` before the
script proving route manifest read, selected route slice read, mandatory
route-slice paths consumed, semantic influence present, dependencies complete,
output phase started, and final script generated. File counts are telemetry
