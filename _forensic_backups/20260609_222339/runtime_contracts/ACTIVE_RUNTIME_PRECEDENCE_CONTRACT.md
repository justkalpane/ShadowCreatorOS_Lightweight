...
## Route-State Active Runtime Addendum

The active runtime truth map is extended by:

```text
runtime/state/route_state_contract.md
runtime/state/route_state.schema.json
runtime/state/active_runtime_truth_map.yaml
runtime/state/route_chain_mode_selector.yaml
registries/route_slices/*.registry_slice.yaml
```

Fresh tasks must resolve a `task_mode` before route expansion. For a plain
script request, the default is `task_mode=script_only`.

Historical files are not active runtime by default:

```text
downloads/chat_transcript.txt=false unless drift_audit_mode=true or explicit_user_reference=true
outputs/missions/**=false unless continue_previous_mission=true, regression_mode=true, or explicit_user_reference=true
proofs/**=false unless validation_mode=true
```

The selected route manifest and selected route slice define active scope for
normal execution. Full registry scans are allowed only in audit, validator, or
rebuild mode.
