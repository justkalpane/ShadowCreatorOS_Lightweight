...
## Semantic Influence Map Law

File reads and route counts are not enough. Every mandatory consumed item must
map to output through `SEMANTIC_INFLUENCE_MAP`.
...
If a mandatory component is selected but semantic use is not proven, classify:

```text
semantic_use_status=NOT_PROVEN
final_status=NEEDS_CONFIRMATION
```

If a route claims `PASS` while mandatory components are only counted or role
summarized, the route fails.
