...
## Route Chain Mode Selector

Every task must resolve one task mode before dependency expansion:
...
For a plain script request, default to `task_mode=script_only` and block
Media Factory contracts, old mission outputs, and chat transcripts until after
`FINAL_SCRIPT` unless the user explicitly requested a downstream route,
mission continuation, or drift audit.

Use `runtime/state/route_chain_mode_selector.yaml` and the selected
`registries/route_slices/*.registry_slice.yaml` to keep the route scoped. For
supported production routes, the route slice is mandatory; block before output
if it is missing. Full registry scans are audit/rebuild behavior, not normal
script generation behavior.

Full repo is the authority; selected route manifest and selected route slice
define the active scope. File counts are telemetry only and never override that
scope.
