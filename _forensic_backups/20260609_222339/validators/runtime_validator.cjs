...
let DossierReader;
try {
  // Prefer the real dossier reader when the engine module is present.
  DossierReader = require('../engine/dossier/dossier_reader');
} catch (error) {
  class FallbackDossierReader {
    constructor(config = {}) {
      this.config = {
  ...
  DossierReader = FallbackDossierReader;
}
