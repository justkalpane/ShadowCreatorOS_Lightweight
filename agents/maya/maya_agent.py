...
# director_binding: Maya (DIR-PRODv1-003) — Visual Production + Creative Visualization + Visual Effects
...
# activation_triggers: route_family in [media_factory_handoff, avatar_video_context] or explicit registry selection; activated when visual production, storyboard, color grading, or cinematic composition tasks are routed.
# upstream_inputs: [scene_brief_packet, script_segment_packet, director_intent_packet]
# downstream_outputs: [scene_prompt_packet, visual_design_brief_packet, color_grading_packet, visual_production_packet]
...
# validator_bindings: [visual_dna_fields_complete, scene_sync_matrix_present, provider_boundary_present]
# fallback_behavior: NEEDS_CONFIRMATION if visual DNA fields are incomplete or scene_brief_packet missing.
# lineage_fields: [scene_id, visual_dna_version, color_palette_id, storyboard_frame_ref, instance_id]
# provider_boundary: provider_execution_allowed=false; local engine execution allowed after approval; provider/media execution remains disabled without approval_packet
...
# handoff_targets: [scene_prompt_packet, visual_design_brief_packet, color_grading_packet, PTR_DIRECTOR_AGENT, PTR_FINAL_SCRIPT_IMAGE, PTR_AGENT_SUBAGENT]
...
# provider_boundary_resolved: provider_execution_allowed=false; local engine allowed after approval_packet
...
# evidence_used_for_resolution: path/pre-contract keyword: visual/cinematic/storyboard/color; component_path=agents/maya/maya_agent.py; component_id=maya_agent; director_file=directors/production/maya.md; registry_entry=agents/AGENT_RUNTIME_REGISTRY.yaml status=DISABLED_MISSING_FILE resolved_by=MAC-06.2B-2J
