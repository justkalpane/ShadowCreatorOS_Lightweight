...
const N8nExecutionReconciler = require('./n8n_execution_reconciler');

class OutputReader {
  constructor() {
    this.reconciler = new N8nExecutionReconciler();
  }
  ...
  async getDossierOutputs(dossierId) {
    const allPackets = this.listPackets();
    const directPackets = allPackets.filter(
      (p) => (p.dossier_id || p.dossier_ref || p.dossierRef) === dossierId
    );
    const executions = await this.reconciler.listDossierExecutions(dossierId);
    const executionPackets = this.reconciler.buildExecutionPackets(dossierId, executions);
    const runtimePackets = await this.reconciler.listDossierRuntimePackets(dossierId);
    const runtimeErrorPackets = await this.reconciler.listDossierRuntimeErrors(dossierId);
    const reconciledOrphans = this.reconciler.reconcileOrphanPackets(dossierId, allPackets, executions);

    const byId = new Map();
    [...directPackets, ...reconciledOrphans, ...executionPackets, ...runtimePackets, ...runtimeErrorPackets].forEach((p) => {
      const key = p.packet_id || p.instance_id || JSON.stringify(p);
      byId.set(key, p);
    });
    const packets = Array.from(byId.values());
    ...
    return {
      ...
      execution_events_count: executionPackets.length,
      runtime_packets_count: runtimePackets.length,
      runtime_errors_count: runtimeErrorPackets.length,
      reconciled_orphan_packets_count: reconciledOrphans.length,
      last_refreshed: nowIso(),
    };
  }
  ...
  async getDossierTimeline(dossierId) {
    ...
    const executions = await this.reconciler.listDossierExecutions(dossierId);
    const executionEvents = executions.map((e) => ({
      type: 'workflow_execution',
      timestamp: e.stopped_at || e.started_at || nowIso(),
      payload: e,
    }));
    const runtimePackets = await this.reconciler.listDossierRuntimePackets(dossierId);
    const runtimePacketEvents = runtimePackets.map((p) => ({
      type: 'runtime_packet',
      timestamp: p.created_at || nowIso(),
      payload: p,
    }));
    const runtimeErrorPackets = await this.reconciler.listDossierRuntimeErrors(dossierId);
    const runtimeErrors = runtimeErrorPackets.map((e) => ({
      type: 'runtime_error',
    ...
    const timeline = [...routeRuns, ...packets, ...executionEvents, ...runtimePacketEvents, ...runtimeErrors, ...errors, ...approvals]
    ...
    return {
      ...
      timeline,
      execution_events_count: executionEvents.length,
      runtime_packets_count: runtimePackets.length,
