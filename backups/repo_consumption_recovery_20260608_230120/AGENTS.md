...
that reads only startup docs, route matrix, route manifest, a few contracts,
directors, and four skills is still shallow and must be blocked. Using
...
Counts without visible path lists are invalid. If
`runtime_contracts_read_count=31` is claimed, the audit must list the runtime
contract paths that were read, and every listed path must exist in the repo.
Do not declare `route_scope_status=PASS` from counts alone.
