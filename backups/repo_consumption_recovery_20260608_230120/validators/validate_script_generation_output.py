...
def validate(text: str) -> ValidationResult:
    ...
    minimum_scope_counts = {
        "startup_docs_read_count": 5,
        "runtime_contracts_read_count": 17,
        "registries_read_count": 7,
        "directors_read_count": 6,
        "agents_read_count": 6,
        "subagents_read_count": 5,
        "skills_read_count": 10,
        "subskills_read_count": 8,
        "total_route_scope_files_read": 68,
    }
    ...
    if route_scope_claims_pass:
        ...
        missing_or_low_counts = [
            f"{key}:{route_scope_counts[key]}<{minimum}"
            for key, minimum in minimum_scope_counts.items()
            if route_scope_counts[key] is None or route_scope_counts[key] < minimum
        ]
        if missing_or_low_counts:
            result.fail("route_scope_file_audit_counts_below_minimum:" + ",".join(missing_or_low_counts))
        listed_count_failures = [
            f"startup_docs:{listed_scope_counts['startup_docs_listed_count']}<5",
            f"runtime_contracts:{listed_scope_counts['runtime_contracts_listed_count']}<17",
            f"registries:{listed_scope_counts['registries_listed_count']}<7",
            f"directors:{listed_scope_counts['directors_listed_count']}<6",
            f"agents:{listed_scope_counts['agents_listed_count']}<6",
            f"subagents:{listed_scope_counts['subagents_listed_count']}<5",
            f"skills:{listed_scope_counts['skills_listed_count']}<10",
            f"subskills:{listed_scope_counts['subskills_listed_count']}<8",
            f"total:{listed_scope_counts['total_route_scope_files_listed']}<68",
        ]
        listed_count_failures = [
            item for item in listed_count_failures if re.search(r":(\d+)<", item)
            and int(re.search(r":(\d+)<", item).group(1)) < int(item.rsplit("<", 1)[1])
        ]
        if listed_count_failures:
            result.fail("route_scope_file_audit_listed_paths_below_minimum:" + ",".join(listed_count_failures))
        ...
        if not mandatory_route_actors_opened:
            result.fail("route_scope_pass_without_individual_actor_file_reads")
