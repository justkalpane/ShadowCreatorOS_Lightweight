...
- `MOTION_GRAPHICS_BATCH_PLAN`: HyperFrames renders, NotebookLM-style panels,
  ...
  For non-cinematic B-roll, each row must include a HyperFrames/local-image
  blueprint with `template_family`, `visual_pattern`, `source_background`,
  `floating_panel_layers`, `kinetic_text_layers`, `lane_assignment`,
  `output_format`, and `proof_gate=hyperframes_proof_json`.
...
## HyperFrames Scene Blueprint Law

When a generator draft uses HyperFrames for non-cinematic B-roll, every
scene-level row or packet that targets this lane must include:
...
Approved `template_family` values:
...
Approved visual-pattern vocabulary may include:

- `local_image_motion_broll_dashboard`
- `notebooklm_clean_dual_panel`
- `floating_evidence_wall`
- `kinetic_lesson_card`
- `transparent_cta_or_highlight_overlay`
- `claim_to_evidence`
- `timeline_roadmap`
- `comparison_card`
- `split_card_horizontal`
- `fullscreen_text_center`
- `gold_title_reveal`

Patterns may map into approved template families. Do not invent a new tool lane
just because the visual treatment changes.

## 30-Second HyperFrames Proof Packet Law

When a task explicitly requests a HyperFrames proof segment, Antigravity test,
or non-cinematic B-roll proof run, the generator draft must emit:
...
The proof packet must use at least four distinct HyperFrames treatments chosen
from:
...
The proof packet may use FFmpeg only for background motion, timing support, or
final assembly support. It must not collapse the proof into a generic
`ffmpeg_ken_burns` sequence while still claiming a HyperFrames-first result.

For `webm_alpha_overlay` scenes, the current production-safe local alpha path
must be declared as:
...
If a scene requests `webm_alpha_overlay` but omits `recommended_overlay_output`
or sets it to `.webm`, the generator draft cannot claim production readiness on
the current local runtime.

## Anti-Fallback Law

If a scene, batch row, or proof packet explicitly requests a HyperFrames family,
the generator draft must not silently downgrade to generic FFmpeg pan/zoom.
...
If these downgrade fields are missing, the output cannot claim HyperFrames lane
coverage, proof completion, or production readiness for that scene.
