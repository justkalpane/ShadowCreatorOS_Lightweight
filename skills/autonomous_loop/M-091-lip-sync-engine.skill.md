# SKL-PH1-lip-sync-engine

## 1. Skill Identity
- **Skill ID:** M-091
- **Skill Name:** Lip Sync Engine
- **Version:** 1.0.0
...
## 2. Purpose
Skill artifact for M-091 (Lip Sync Engine), generated using the latest repository skill DNA template and runtime contract. This skill preserves the same structure, routing semantics, escalation behavior, and mutation law as the current compliant skills.

## 3. DNA Injection
- **Role Definition:** lip-sync-engine_executor
...
- **Downstream Handoff:** lip-sync-engine_packet -> downstream workflow chain
...
3. Execute core transformation logic for M-091
...
```json
{
  "instance_id": "M-091-[timestamp]",
  "artifact_family": "lip-sync-engine_packet",
  ...
  "payload": {
    "skill_id": "M-091",
    "skill_name": "Lip Sync Engine",
    "result": {}
  }
}
```
**Write Targets:**
- dossier.governance_vein.lip-sync-engine (append_to_array)
...
**Writes:**
- dossier.governance_vein.lip-sync-engine (append_only)
...
**Acceptance Tests:**
- TEST-PH1-M091-001: valid input produces CREATED packet
- TEST-PH1-M091-002: invalid input escalates to WF-900
- TEST-PH1-M091-003: dossier patch is additive only
...
component_id: M-091-lip-sync-engine.skill
component_layer: SKILL
component_name: M 091 Lip Sync Engine.Skill
...
evidence_used_for_resolution: path/pre-contract keyword: lineage/trace; component_path=skills/autonomous_loop/M-091-lip-sync-engine.skill.md; component_id=M-091-lip-sync-engine.skill
