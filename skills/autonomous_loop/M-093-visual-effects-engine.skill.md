# SKL-PH1-VISUAL-EFFECTS-ENGINE

## 1. Skill Identity
- **Skill ID:** M-093
- **Skill Name:** Visual Effects Engine
- **Version:** 1.0.0
...
## 2. Purpose
Skill artifact for M-093 (Visual Effects Engine), generated using the latest repository skill DNA template and runtime contract. This skill preserves the same structure, routing semantics, escalation behavior, and mutation law as the current compliant skills.

## 3. DNA Injection
- **Role Definition:** visual-effects-engine_executor
...
- **Downstream Handoff:** visual-effects-engine_packet -> downstream workflow chain
...
3. Execute core transformation logic for M-093
...
```json
{
  "instance_id": "M-093-[timestamp]",
  "artifact_family": "visual-effects-engine_packet",
  ...
  "payload": {
    "skill_id": "M-093",
    "skill_name": "Visual Effects Engine",
    "result": {}
  }
}
```
**Write Targets:**
- dossier.governance_vein.visual-effects-engine (append_to_array)
...
**Writes:**
- dossier.governance_vein.visual-effects-engine (append_only)
...
**Acceptance Tests:**
- TEST-PH1-M093-001: valid input produces CREATED packet
- TEST-PH1-M093-002: invalid input escalates to WF-900
- TEST-PH1-M093-003: dossier patch is additive only
...
component_id: M-093-visual-effects-engine.skill
component_layer: SKILL
component_name: M 093 Visual Effects Engine.Skill
route_families: [lineage_summary, approval_gate]
activation_triggers: route_family in [script_generation, topic_discovery, visual_context, quality_gate] or explicit registry selection; mark media_quality_gate_profile only when route_family is unknown.
...
evidence_used_for_resolution: path/pre-contract keyword: lineage/trace; component_path=skills/autonomous_loop/M-093-visual-effects-engine.skill.md; component_id=M-093-visual-effects-engine.skill
