# SKL-PH1-MOTION-GRAPHICS-ENGINE

## 1. Skill Identity
- **Skill ID:** M-095
- **Skill Name:** Motion Graphics Engine
- **Version:** 1.0.0
...
## 2. Purpose
Skill artifact for M-095 (Motion Graphics Engine), generated using the latest repository skill DNA template and runtime contract. This skill preserves the same structure, routing semantics, escalation behavior, and mutation law as the current compliant skills.

## 3. DNA Injection
- **Role Definition:** motion-graphics-engine_executor
...
- **Downstream Handoff:** motion-graphics-engine_packet -> downstream workflow chain
...
3. Execute core transformation logic for M-095
...
```json
{
  "instance_id": "M-095-[timestamp]",
  "artifact_family": "motion-graphics-engine_packet",
  ...
  "payload": {
    "skill_id": "M-095",
    "skill_name": "Motion Graphics Engine",
    "result": {}
  }
}
```
**Write Targets:**
- dossier.governance_vein.motion-graphics-engine (append_to_array)
...
**Writes:**
- dossier.governance_vein.motion-graphics-engine (append_only)
...
**Acceptance Tests:**
- TEST-PH1-M095-001: valid input produces CREATED packet
- TEST-PH1-M095-002: invalid input escalates to WF-900
- TEST-PH1-M095-003: dossier patch is additive only
...
component_id: M-095-motion-graphics-engine.skill
component_layer: SKILL
component_name: M 095 Motion Graphics Engine.Skill
route_families: [lineage_summary, approval_gate]
activation_triggers: route_family in [script_generation, topic_discovery, quality_gate, lineage_summary] or explicit registry selection; mark media_quality_gate_profile only when route_family is unknown.
...
evidence_used_for_resolution: path/pre-contract keyword: lineage/trace; component_path=skills/autonomous_loop/M-095-motion-graphics-engine.skill.md; component_id=M-095-motion-graphics-engine.skill
