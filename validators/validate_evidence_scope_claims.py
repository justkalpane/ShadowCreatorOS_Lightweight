...
from lib.claim_normalizer import first_value, has_pass_claim, parse_claims

ALLOWED = {"current_cycle", "previous_cycle_with_step_reference", "persisted_route_state_with_hash"}


def parse(path: Path) -> dict[str, str]:
    return {key: values[-1] for key, values in parse_claims(path.read_text()).items() if values}


def check(path: Path) -> bool:
    data = parse(path)
    claims = parse_claims(path.read_text())
    if not has_pass_claim(claims):
        return True
    return (
...
def self_test() -> int:
    ...
    bad_alias = Path("validators/fixtures/bad/bad_latest_antigravity_alias_fake_pass.md")
    gold_alias = Path("validators/fixtures/gold/gold_alias_claims_with_complete_evidence.md")
    ok = check(good) and check(current) and not check(bad_memory) and not check(bad_scope) and not check(bad_alias) and check(gold_alias)
    ...
    print(f"bad_alias_fixture: {'FAIL' if not check(bad_alias) else 'PASS'}")
    print(f"gold_alias_fixture: {'PASS' if check(gold_alias) else 'FAIL'}")
    return 0 if ok else 1
